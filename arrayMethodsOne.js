...
// CODE HERE
const totalPopulation  = populations.reduce((acc, current) => {return acc + current})
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter((element) => {return element.CP > 200})
...
// CODE HERE
const totals = orders.map((element) => {return element.price * (1 + element.tax)})
...
// CODE HERE
let justBob = purchases.filter((element) => {return element.owner === "Bob"})

const bobsTotal = justBob.reduce((acc, current) => acc + current.price, 0)
