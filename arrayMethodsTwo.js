...
// CODE HERE
let newArray = numbers.map((element) => {return element / 2})
...
// CODE HERE
let iCities = cities.filter((element) => {return element[element.length - 1] === 'i'})
...
fruits.forEach((element, index) => {
  if(element.color === 'red'){
    console.log(`The fruit with the ${index} is an apple.`)
  } else if (element.color === 'orange'){
    console.log(`The fruit with the index of ${index} is an orange.`)
  } else {
...
The fruit with the 3 is an apple.
*/
...
// CODE HERE
let meatObjects = foods.filter((element) => element.tags.includes("meat"))


console.log(meatObjects)

// THE TOTAL
let totalPrice = meatObjects.reduce((acc, current) => acc + current.price, 0)
